...
class Allcategorydisplay extends StatefulWidget {
  String api_url, appbar_name, category;

  Allcategorydisplay(
      {required this.api_url,
      required this.appbar_name,
      required this.category});

  @override
  State<Allcategorydisplay> createState() => _AllcategorydisplayState();
}

class _AllcategorydisplayState extends State<Allcategorydisplay> {
  int allmoviewswebdatalength = 0;
  String? appbar_name, api_url, category;

  Future<List<allmoviesdata>> fetchmovies() async {
    try {
      var response = await http.post(Uri.parse(api_url!), body: {
